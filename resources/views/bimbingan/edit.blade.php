...
@section('main')
                ...
                <div class="card mb-3">
                    <div class="card-header bg-primary d-flex justify-content-between align-items-center">
                        <strong class="bg-primary text-light">Edit Progres Bimbingan 
                    ...
                    <div class="card-body border-bottom py-2">
                        {!! Form::open(['url' => 'bimbingan/'. $bimbingan->id, 'method' => 'patch']) !!}
                            {{ csrf_field() }}
                            {!! Form::hidden('id_mahasiswa', Session::get('id')) !!}

                            @if(Request::segment(2) === 'edit-proposal')
                                {!! Form::hidden('bimbingan', 'proposal') !!}
                            @elseif(Request::segment(2) === 'edit-hasil')
                                {!! Form::hidden('bimbingan', 'hasil') !!}
                            @elseif(Request::segment(2) === 'edit-sidang-skripsi')
                                {!! Form::hidden('bimbingan', 'sidang-skripsi') !!}
                            @elseif(Request::segment(2) === 'edit-kerja-praktek')

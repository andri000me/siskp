...
<body>
        ...
        <?php 
            $status = $nilaiUjian->status; 
            $total = $nilaiUjian->total; 
            $nilai_huruf = $nilaiUjian->nilai_huruf; 
        ?>
        @endforeach
    </table>
    ...
    <p class="my-0 py-0">
        ...
        @else
            TIDAK LULUS
        @endif
        </strong> dengan Total Nilai {{ $total }} dan Grade {{ $nilai_huruf }}
    </p>
        <?php $i=1 ?>
        @foreach($dospeng as $dosenPenguji)
                <div class="float-left mx-5 px-3">
                <p>&nbsp;&nbsp; Penguji {{ $i }}</p>
                @if(!empty($dosenPenguji[0]->dosen->tanda_tangan))
                <img src="{{ asset('assets/ttd/' . $dosenPenguji[0]->dosen->tanda_tangan) }}" style="width:25%; padding-left:-30px;" class="img pt-2">
                @endif
                <br> <br> <br>
                <p style="font-size:10px;">{{ implode(' ', array_slice(explode(' ', $dosenPenguji[0]->dosen->nama), 0, 3)) }} </p>
            </div>
        <?php $i++ ?>
        @endforeach
